...
import { createBank, type BankState } from './economy'
...
/** Fixed daily tick order: research unlocks feed design, staff sets capacity, production fills stock, marketing/market sell it. Mirrors SimulationRunner.cs's system order exactly. */
export function advanceOneDay(world: World, catalog: Catalog, config: GameConfig): void {
  onResearchDayTick(world.research, catalog.researchNodes, config.researchPointsPerDay)
  onStaffDayTick(world.staff, world.bank)
  onProductionDayTick(world.vehicles.models, world.bank, productionSpeedBonusPercent(world.staff))
...
export function tickWorld(world: World, catalog: Catalog, config: GameConfig, deltaSeconds: number): number {
  return tickTime(world.time, deltaSeconds, () => advanceOneDay(world, catalog, config))
}
